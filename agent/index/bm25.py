...
class SimpleBM25:
    ...
    def get_score_items(
        ...
        selected_terms = self._select_query_terms(query_tokens)
        if allowed_doc_indices:
            direct_work = len(allowed_doc_indices) * len(selected_terms)
            posting_work = sum(len(self.postings.get(token, ())) for token in selected_terms)
            if direct_work <= posting_work:
                return [
                    (doc_idx, score)
                    for doc_idx in sorted(allowed_doc_indices)
                    if (score := self.score(selected_terms, doc_idx)) > 0
                ]

        scores: dict[int, float] = {}
        for token in selected_terms:
            idf = self.idf.get(token, 0.0)
            if idf <= 0:
                continue
            for doc_idx, tf in self.postings.get(token, []):
                if allowed_doc_indices and doc_idx not in allowed_doc_indices:
                    continue
                doc_len = self.doc_lens[doc_idx] or 1
                denom = tf + self.k1 * (1 - self.b + self.b * doc_len / max(1e-9, self.avgdl))
    ...
    def _select_query_terms(self, query_tokens: list[str]) -> list[str]:
        ...
        ordered = sorted(unique, key=lambda token: (-self.idf.get(token, 0.0), token))
        return ordered[: self.max_query_terms]
...
class BM25SearchIndex:
    ...
    def __init__(
        ...
        self.chunk_indices_by_doc_id: dict[str, set[int]] = defaultdict(set)
        ...
        for chunk_idx, chunk in enumerate(self.chunks):
            self.chunk_indices_by_doc_id[chunk.doc_id].add(chunk_idx)
    ...
    def search(
        ...
        allowed_doc_indices = self._allowed_doc_indices(filter_doc_ids, filter_chunk_types)
        ranked = sorted(
            self.engine.get_score_items(query_tokens, allowed_doc_indices=allowed_doc_indices),
    ...
    def _field_aware_search(
        ...
        allowed_doc_indices = self._allowed_doc_indices(filter_doc_ids, filter_chunk_types)
        component_scores = self._collect_component_scores(
            query_tokens,
    ...
    def _collect_component_scores(
        ...
        *,
        ...
        for idx, score in self.engine.get_score_items(query_tokens, allowed_doc_indices=allowed_doc_indices):
            if score > 0:
                combined[idx]["base"] = float(score)

        for field in ("title", "section", "clause_id", "caption", "structured"):
            for idx, score in self.field_engines[field].get_score_items(
                query_tokens,
                allowed_doc_indices=allowed_doc_indices,
            ):
                if score > 0:
        ...
        if query_numbers:
            ...
            for idx, score in self.field_engines["numbers"].get_score_items(
                number_tokens,
                allowed_doc_indices=allowed_doc_indices,
            ):
                if score > 0:
        ...
        if query_dates:
            ...
            for idx, score in self.field_engines["dates"].get_score_items(
                date_tokens,
                allowed_doc_indices=allowed_doc_indices,
            ):
                if score > 0:
    ...
    def _allowed_doc_indices(
        ...
        if filter_doc_ids:
            ...
            for doc_id in filter_doc_ids:
                candidates.update(self.chunk_indices_by_doc_id.get(doc_id, ()))
        else:
        ...
        if filter_chunk_types:
            candidates = {
                ...
                if str(self.chunks[idx].metadata.get("chunk_type", "text")) in filter_chunk_types
            }
        return candidates
