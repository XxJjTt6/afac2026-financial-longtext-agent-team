...
from agent_team_b1.full_accuracy_audit_v30 import (
    ANSWER_OVERRIDES_V30,
    REASONING_OVERRIDES_V30,
    apply_full_accuracy_audit_v30,
)
...
def build_v30_artifacts(
    *,
    ...
    if len(source_rows) != len(source_by_qid):
        raise ValueError("V30 来源存在重复或空 qid")
    if len(specs) != 100 or set(source_by_qid) != set(expected_order):
        raise ValueError("V30 来源必须与官方模板的100个 qid 完全一致")

    audited_rows = apply_full_accuracy_audit_v30(source_rows)
    ...
    for row in ordered:
        ...
        if row.get("token_usage") != source_by_qid[qid].get("token_usage"):
            raise ValueError(f"{qid} 的 V30 审计不得改动来源 API usage")
    ...
    for qid, answers in ANSWER_OVERRIDES_V30.items():
        ...
        if row["answers"] != answers:
            raise ValueError(f"{qid} 未应用 V30 答案修复")
        if row["reasoning"] != REASONING_OVERRIDES_V30[qid]:
            raise ValueError(f"{qid} 未应用 V30 reasoning")
        answer_text = "；".join(answers)
        if answer_text not in str(row["reasoning"]).rsplit("结论", 1)[-1]:
    ...
    answer_deltas = {
        ...
        for qid in ANSWER_OVERRIDES_V30
        if source_by_qid[qid]["answers"] != audited_by_qid[qid]["answers"]
    }
    if len(answer_deltas) != len(ANSWER_OVERRIDES_V30):
        raise ValueError("V30 应当产生8道实质答案修正")

    manifest: dict[str, Any] = {
        "version": "b1_full_accuracy_audit_v30",
        ...
        "reasoning_repair_count": len(REASONING_OVERRIDES_V30),
