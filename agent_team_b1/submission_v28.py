...
from agent_team_b1.accuracy_audit_v28 import (
    ANSWER_OVERRIDES_V28,
    REASONING_OVERRIDES_V28,
    apply_accuracy_audit_v28,
)
...
def build_v28_artifacts(
    *,
    ...
    if len(source_rows) != len(source_by_qid):
        raise ValueError("V28 来源存在重复或空 qid")
    if len(specs) != 100 or set(source_by_qid) != set(expected_order):
        raise ValueError("V28 来源必须与官方模板的100个 qid 完全一致")

    audited_rows = apply_accuracy_audit_v28(source_rows)
    audited_by_qid = {str(row["qid"]): row for row in audited_rows}
    ordered = [audited_by_qid[qid] for qid in expected_order]

    for row in ordered:
        qid = str(row["qid"])
        source_usage = source_by_qid[qid].get("token_usage")
        if row.get("token_usage") != source_usage:
            raise ValueError(f"{qid} 的 V28 审计不得改动来源 API usage")
    ...
    for qid, answers in ANSWER_OVERRIDES_V28.items():
        row = audited_by_qid[qid]
        if row["answers"] != answers:
            raise ValueError(f"{qid} 未应用 V28 锁定答案")
        conclusion = str(row["reasoning"]).rsplit("结论", 1)[-1]
        if answers[0] not in conclusion:
            raise ValueError(f"{qid} 的 reasoning 结论与答案不一致")
    if audited_by_qid["reg_b_001"]["reasoning"] != REASONING_OVERRIDES_V28["reg_b_001"]:
        raise ValueError("reg_b_001 的矛盾 reasoning 未修复")
    ...
    answer_deltas = {
        ...
        for qid in ANSWER_OVERRIDES_V28
        if source_by_qid[qid]["answers"] != audited_by_qid[qid]["answers"]
    }
    if len(answer_deltas) != len(ANSWER_OVERRIDES_V28):
        raise ValueError("V28 应当产生11道实质答案修正")

    manifest: dict[str, Any] = {
        "version": "b1_accuracy_audit_v28",
        ...
        "reasoning_repair_count": len(REASONING_OVERRIDES_V28),
